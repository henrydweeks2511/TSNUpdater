var ChooseUpdateType = ChooseUpdateType || {};

ChooseUpdateType.itemViewModel = function (moduleId, resx) {
    var service = {
        path: "ChooseUpdateType",
    ...
    var loadUsers = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            var result = data[i];
            var user = new ChooseUpdateType.user(result.id, result.name);
...
ChooseUpdateType.user = function (id, name) {
    this.id = id;
    this.name = name;
}